...
if (isset($_POST['from'])) {
  ...
  if ($report == 'stockin') {
    $status = 'stockin';
    $title = 'Garage Report on stock in from ' .  $yearfrom .'/'. $monthfrom.'/'.$dayfrom.' to '. $yearto.'/'. $monthto.'/'. $dayto;
    ...
    $ok = mysqli_query($connection, "SELECT product.pid,product.pname,historic.quantity,historic.price,historic.total,historic.year,historic.month,historic.day,historic.time
        FROM product,historic WHERE product.pid=historic.pid and status='$status'  and  `year` = $yearfrom 
  AND `month` BETWEEN $monthfrom AND $monthto
  AND `day` >= $dayfrom AND `day`<=$dayto order by id desc");
    ...
    $status = 'stockout';
    $title = 'Garage Report on stock out from ' .  $yearfrom .'/'. $monthfrom.'/'.$dayfrom.' to '. $yearto.'/'. $monthto.'/'. $dayto;
  ...
  AND `day` >= $dayfrom AND `day`<=$dayto  order by id desc");
    ...
    $status = 'general';
    $title = 'Garage General from ' .  $yearfrom .'/'. $monthfrom.'/'.$dayfrom.' to '. $yearto.'/'. $monthto.'/'. $dayto;

    $ok = mysqli_query($connection, "SELECT product.pid,product.pname,historic.quantity,historic.price,historic.total,historic.year,historic.month,historic.day,historic.time,historic.profit
          FROM product,historic WHERE product.pid=historic.pid and  `year` = $yearfrom 
  AND `month` BETWEEN $monthfrom AND $monthto
  AND `day` >= $dayfrom AND `day`<=$dayto  order by id desc");
...
<body>
  ...
  <main id="main" class="main">
    ...
    <section class="section dashboard" style='margin-top:0cm'>
      <div class="row">
